...
const unsigned int no_points = 3;
const unsigned int no_points_on_circle = 30;
...
int main(int argc, char** argv)
{
    ...
    float point_y = 0.2;
     // set the args values
    ciErrNum  = clSetKernelArg(ckKernel_color_change, 0, sizeof(float), &point_x);
    ciErrNum |= clSetKernelArg(ckKernel_color_change, 1, sizeof(float), &point_y);
    ciErrNum |= clSetKernelArg(ckKernel_color_change, 2, sizeof(cl_mem), (void *) &vbo_cl_colors);
    ciErrNum |= clSetKernelArg(ckKernel_color_change, 3, sizeof(cl_mem), (void *) &vbo_cl_mask);
    shrCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
...
{
    // create VBO
    unsigned int size = no_points_on_circle * 2 * sizeof(float);
    if(!bQATest)
    {
...
{
    // create VBO
    unsigned int size = no_points_on_circle * 4 * sizeof(float);
    if(!bQATest)
    {
        // create buffer object
        glGenBuffers(1, vbo);
...
void DisplayGL()
{
    ...
    glPointSize(7);
    glDrawArrays(GL_POINTS, 0, no_points_on_circle);
