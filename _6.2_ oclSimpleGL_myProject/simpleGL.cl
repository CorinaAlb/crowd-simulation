...
__kernel void move_to_target_path_faithful(__global float2* pos, __global float2* collision_map,
                                        __global float2* target, __global int* path_faithful)
{
    unsigned int gid = get_global_id(0);
...
//    }
    ...
    pos[gid].x = pos[gid].x + 0.005 * sign_x * (obstacle_exists == 0 || path_faithful[gid] == 1) + back_off.x * (obstacle_exists != 0 && path_faithful[gid] == 0);
    pos[gid].y = pos[gid].y + 0.005 * sign_y * (obstacle_exists == 0 || path_faithful[gid] == 1) + back_off.y * (obstacle_exists != 0 && path_faithful[gid] == 0);
}
