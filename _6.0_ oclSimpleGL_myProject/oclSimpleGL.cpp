...
cl_kernel ckKernel;
cl_kernel ckKernel_collision_map;
...
int main(int argc, char** argv)
{
    ...
    shrCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
    ckKernel_collision_map = clCreateKernel(cpProgram, "collision_detection", &ciErrNum);
    shrCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
    ...
    ciErrNum  = clSetKernelArg(ckKernel_collision_map, 0, sizeof(cl_mem), (void *) &vbo_cl_positions);
    ciErrNum  = clSetKernelArg(ckKernel_collision_map, 1, sizeof(cl_mem), (void *) &vbo_cl_collision_map);
    shrCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
...
void runKernel()
{
    ...
    shrCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
#endif
    size_t szGlobalWorkSize[] = {(size_t) no_points - NO_OBSTACLES, 1};
    ...
    shrCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
    ciErrNum = clEnqueueNDRangeKernel(cqCommandQueue, ckKernel_collision_map, 1, NULL, szGlobalWorkSizeCollision, NULL, 0, 0, 0 );
    shrCheckErrorEX(ciErrNum, CL_SUCCESS, pCleanup);
...
void Cleanup(int iExitCode)
{
    ...
    if(ckKernel_collision_map)       clReleaseKernel(ckKernel_collision_map);
