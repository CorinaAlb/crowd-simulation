...
__kernel void compute_velocity(__global float2* position, __global float2* old_position, __global float2* velocity)
{
    ...
    int velocity_zero_x = velocity[gid].x > 0.0f;
    int velocity_zero_y = velocity[gid].y > 0.0f;

    velocity[gid].x -= velocity_zero_x * BOUNCING_SPEED_MODIFIER * (outside == 1);
    velocity[gid].y -= velocity_zero_y * BOUNCING_SPEED_MODIFIER * (outside == 1);
}
